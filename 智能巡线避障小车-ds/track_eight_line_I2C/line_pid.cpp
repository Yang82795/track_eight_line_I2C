...
/* =========================================================================
 *  巡线控制核心 (8路I2C红外 + 连续质心偏差 + 位置式PD + 软里程 + 事件上报)
 *
 *  传感器约定: 1 = 白底, 0 = 压到黑线 (与模板代码一致, 上车前用串口确认)
 *  x1~x8 从左往右(x1在最左), 车头朝前
 *
 *  为什么不用原来那种"枚举20多种位型"的写法:
 *   1) 8路只有 2~3 路压线, 枚举必然漏; 质心法对任何位型都连续可算;
 *   2) 发夹弯处会出现"8路全黑", 枚举法只能"保持上一次", 无法区分
 *      "终点横线"和"发夹黑块", 这里用事件位上报给主状态机判断。
 * ========================================================================= */

/* -------------------- 可调参数(现场主要调这几个) -------------------- */
#define SENSOR_ADDR      (0x12)    // 巡线模块I2C地址
#define SENSOR_REG       (0x30)    // 读取寄存器
#define SENSOR_READ_US   (0)       // 读命令与读数据间隔(us), 读数不稳时改200~500

#define SENSOR_PITCH_MM  (11.0f)   // 【实测】相邻两路红外间距(mm)
#define BLACK_IS_ZERO    (1)       // 1: 0=压黑线(模板默认); 0: 1=压黑线

#define Speed_Line       (100)     // 直道基准速度(PWM)
#define Speed_Min        (85)      // 普通弯道最低速度(PWM)
#define Speed_Crest      (75)      // 发夹/黑块区速度(PWM)
#define Speed_Lost       (75)      // 丢线保持速度(PWM)
#define K_SLOW           (0.35f)   // 每 mm 偏差降速(PWM/mm)
#define KPx              (3.0f)    // 比例 PWM/mm (模板≈4.1; 太大易摆头, 太小切弯不足)
#define KDx              (4.0f)    // 微分 PWM/(mm/周期)
#define KIx              (0.0f)    // 积分(一般保持0, 防止绕圈饱和)
#define KD_FILT          (0.35f)   // D项一阶滤波系数
#define STEER_MAX        (95)      // 转向量限幅(PWM): 与基准速度配合, 偏差很大时才让内轮反转
...
#define MM_PER_PWM_SEC   (0.0032f) // 【标定】每1PWM对应的速度(m/s), 用于软里程

#define FULL_TH          (8)       // "8路全黑"判定门限
#define WIDE_TH          (5)       // "黑线过宽"判定门限

/* ------------------------------ 全局 ------------------------------ */
uint8_t x1,x2,x3,x4,x5,x6,x7,x8;   // 保留原接口: 从左往右

static float g_err_valid = 0;      // 上一次"有效"偏差(mm, 只有正常压线才更新)
static float g_err_prev  = 0;      // PID用: 上一周期偏差(已减偏置)
static float g_d_filt    = 0;      // D项滤波值
static float g_integral  = 0;      // I项
static float g_env       = 0;      // 偏差包络(|err|峰值保持+衰减)
static int   g_offset    = 0;      // 目标偏置(mm)
static int8_t g_dir_mem  = 1;      // 最近一次"明显偏差"的方向(1右/-1左), 丢线找回用
...
/**
 * @brief 读一次8路巡线模块(I2C), 返回掩码 bit7..bit0 = x1..x8
 */
uint8_t line_read(void)
{
    ...
    {
        m = last_mask;      // I2C读失败时沿用上次结果, 避免误判"丢线"
    }
...
/**
 * @brief 计算本周期偏差并给出事件位
 * @param ev: 返回 LINE_EV_LOST / LINE_EV_WIDE / LINE_EV_FULL 组合
 */
static float line_calc_err(uint8_t mask, uint8_t *ev)
{
    ...
    *ev = LINE_EV_NONE;
    ...
    if (n == 0)
    {
        /* 8路全白: 丢失循迹线(发夹/急弯冲出去), 沿上次方向继续加大转向 */
        *ev = LINE_EV_LOST;
        e = g_err_valid * ERR_LAST_GAIN;
    }
    else if (n >= FULL_TH)
    {
        /* 8路全黑: 只有两种可能——终点横线(500mm) 或 发夹顶部的黑块。
           这里先按"发夹"处理: 保持并放大上次转向, 减速;
           是否是终点线由主状态机用"先刹后判"决定。 */
        *ev = LINE_EV_FULL | LINE_EV_WIDE;
        e = g_err_valid * 1.3f;
    }
    else if (n >= WIDE_TH)
    {
        /* 黑线过宽: 发夹两条腿同时压到, 或车斜跨黑块 */
        *ev = LINE_EV_WIDE;
        e = g_err_valid * 1.15f;
    }
    else
 ...
 *        约定: Set_speed(基准, 转向) → 左轮=基准+转向, 右轮=基准-转向
 */
static int line_calc_steer(float err)
{
    ...
    if (g_integral >  30.0f) g_integral =  30.0f;
    if (g_integral < -30.0f) g_integral = -30.0f;
...
static void line_commit(float err, int base)
{
    ...
    /* 软里程: 没有编码器, 用"指令速度×时间"估算, 用于终点线判定的里程闸门 */
...
void line_set_offset(int mm)
{
    g_offset = mm;
}

int line_last_error_mm(void)
{
    return (int)g_err_valid;
}

/**
 * @brief 最近一次"明显偏差"的方向: 1=线在右(应右转), -1=线在左
 *        丢线后原地找线的旋转方向用它, 比用瞬时偏差稳
 */
int line_last_dir(void)
{
    return (int)g_dir_mem;
}

int line_err_envelope_mm(void)
{
    return (int)g_env;
}

uint32_t line_dist_mm(void)
{
    return g_dist;
}
